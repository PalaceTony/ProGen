...
export MODEL_NAME="full_run_pems08"  
export DATASET="PEMS08"
export DEVICE_FULL="4a800-01"
export SAMPLING_FIRST_BATCH=128 
...
torchrun --nproc_per_node=2 src/model/${MODEL_NAME}/v011_run_sde.py \
"hyperopt=False" \
"hyperopt_trial=300" \
...
"wandb_mode='dryrun'" \
"excel_notes='${DEVICE_FULL}. ${MODEL_NAME}. NewSDE used for sampling only. no diffusion. Sampling. ${DATASET}. ${SAMPLES} samples. ${SAMPLING_FIRST_BATCH_VALUE} batches.'" \
"n_samples=${SAMPLES}" \
