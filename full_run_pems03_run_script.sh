...
export MODEL_NAME="full_run_pems03"  
export DATASET="PEMS03"
export DEVICE_FULL="8a8001-01234567"
export SAMPLING_FIRST_BATCH=32 
...
export OUTPUT_DIR="outputs/sampling/${DATE}/${TIME}"
...
torchrun --nproc_per_node=8 src/model/${MODEL_NAME}/v011_run_sde.py \
"hyperopt=False" \
"hyperopt_trial=300" \
...
"wandb_mode='dryrun'" \
"excel_notes='${DEVICE_FULL}. ${MODEL_NAME}. NewSDE used for sampling only. no diffusion. Sampling. ${DATASET}. ${SAMPLES} samples.'" \
"n_samples=${SAMPLES}" \
...
# "dataset.dif_model.model.num_scales=2" \
# "test_saved_data=True" \
