...
def optimization_manager(args):
    ...
    def optimize_fn(
        ...
        warmup=args.dataset.dif_model.model.warmup,
        grad_clip=args.dataset.dif_model.model.grad_clip,
    ):
...
def get_sde_loss_fn(
    ...
    def loss_fn(model, batch, adj, cond, pos_w, pos_d):
        ...
        score_fn = mutils.get_score_fn(
            sde, model, adj, cond, pos_w, pos_d, train=train, continuous=continuous
        )
        t = torch.rand(batch.shape[0], device=batch.device) * (sde.T - eps) + eps
        z = torch.randn_like(batch)
        mean, std = sde.marginal_prob(batch, t)
        perturbed_future = mean + std[:, None, None, None] * z
        score = score_fn(perturbed_future, t)
...
def get_step_fn(
    ...
    def step_fn(state, batch, adj, cond, pos_w, pos_d):
        ...
        model = state["model"]
        if train:
            loss = loss_fn(model, batch, adj, cond, pos_w, pos_d)
        else:
            with torch.no_grad():
                ema = state["ema"]
