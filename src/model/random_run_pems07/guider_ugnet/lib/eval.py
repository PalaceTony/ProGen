...
class Metric(object):
    ...
    def log_lst(self, epoch=None, sep=","):
        ...
        for i in index:
            message_lst.append(
                f"{i},{self.multi_step_str(obj=i, sep=sep, epoch=epoch)}"
            )
        return message_lst
...
#     """
#     # forecast: (64, 2, 12, 170, 1), target: (64, 12, 170, 1)
...
def calc_quantile_CRPS(target, forecast, eval_points):
    ...
    denom = calc_denominator(target, eval_points)
    CRPS = 0
    q_preds = torch.quantile(forecast, quantiles, dim=1)
    for i, q in enumerate(quantiles):
        q_loss = quantile_loss(target, q_preds[i], q, eval_points)
        CRPS += q_loss / denom
    return CRPS.item() / len(quantiles)
