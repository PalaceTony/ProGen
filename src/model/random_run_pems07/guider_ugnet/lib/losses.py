...
def optimization_manager(args):
    ...
    def optimize_fn(
        ...
        step,
        lr=args.lr_init,
        warmup=args.dif_model.model.warmup,
        grad_clip=args.dif_model.model.grad_clip,
    ):
...
def get_sde_loss_fn(
    ...
    def loss_fn(model, batch, adj, cond, guider, label):
        ...
        score_fn = mutils.get_score_fn(
            sde, model, adj, cond, train=train, continuous=continuous
        )
...
def get_step_fn(
    ...
    def step_fn(state, batch, adj, cond, guider, label, args):
        ...
        """
        model = (
            state["model"].eps_model
            if not args.use_distributed
            else state["model"].module.eps_model
        )
        if train:
            loss = loss_fn(model, batch, adj, cond, guider, label)
        else:
            with torch.no_grad():
                ema = state["ema"]
