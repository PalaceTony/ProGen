...
def All_Metrics(predictions_det, predictions_prob, true, mask1, mask2):
    # mask1 filter the very small value, mask2 filter the value lower than a defined threshold
    assert type(predictions_det) == type(true) == type(predictions_prob)
    mae = MAE_torch(predictions_det, true, mask1)
    rmse = RMSE_torch(predictions_det, true, mask1)
    mape = MAPE_torch(predictions_det, true, mask2)
    crps = calc_quantile_CRPS(
        true,
        predictions_prob,
    )
    mis = calc_mis(true, predictions_prob)

    return mae, rmse, mape, crps, mis
...
def calc_quantile_CRPS(target, forecast):
    """
