...
class Trainer(object):
    ...
    def sampling(self, train_metrics=None):
        ...
        all_indices = []
        with torch.no_grad():
            if not self.args.test_saved_data:
                for batch_index, (source, label, pos_w, pos_d, idx) in enumerate(
                    ...
                    source = source[
                        ..., : self.args.dataset.dif_model.AGCRN.input_dim
                    ].to(self.device)
                    label = label[
                        ..., : self.args.dataset.dif_model.AGCRN.output_dim
                    ].to(self.device)
                    pos_w = pos_w.to(self.device)
                    pos_d = pos_d.to(self.device)
                    ...
                    predictions_samples = (
                        predictions.reshape(
                            -1, self.args.n_samples, *predictions.shape[1:]
                    ...
                    with open(
                        os.path.join(
                            self.args.log_dir,
                            f"predictions_samples_{self.local_rank}_{batch_index}.pkl",
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(predictions_samples, f)
                    with open(
                        os.path.join(
                            self.args.log_dir,
                            f"best_predictions_samples_{self.local_rank}_{batch_index}.pkl",
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(best_predictions_samples, f)
                    with open(
                        os.path.join(
                            self.args.log_dir,
                            f"labels_{self.local_rank}_{batch_index}.pkl",
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(label, f)
                ...
                with open(
                    os.path.join(
                        self.args.log_dir, f"predictions_list_{self.local_rank}.pkl"
                    ),
                    "wb",
                ) as f:
                    pickle.dump(predictions_lists, f)
                with open(
                    os.path.join(
                        self.args.log_dir,
                        f"best_predictions_list_{self.local_rank}.pkl",
                    ),
                    "wb",
                ) as f:
                    pickle.dump(best_predictions_lists, f)
                with open(
                    os.path.join(
                        self.args.log_dir, f"labels_list_{self.local_rank}.pkl"
                    ),
                    "wb",
                ) as f:
                    pickle.dump(labels_lists, f)

                labels = torch.cat(labels_lists, dim=0)  # (B, T, N, D)
                predictions = torch.cat(
                    predictions_lists, dim=0
                )  # (B, n_samples, T, N, D)
                best_predictions = torch.cat(best_predictions_lists, dim=0)

            else:

                def load_pickle_lists(pattern, device):
                    ...
                    for f in files:
                        with open(f, "rb") as file:
                            ...
                            tensor_list = [
                                tensor.to(device) for tensor in tensor_list
                            ]  # Move tensors to the same device
                            all_tensors.extend(tensor_list)
                    return torch.cat(all_tensors, dim=0)

                predictions_lists = load_pickle_lists(
                    os.path.join(
                        "outputs/sampling/2024-07-03/11-37-55", "predictions_list_*.pkl"
                    ),
                    self.device,
                )
                best_predictions_lists = load_pickle_lists(
                    os.path.join(
                        "outputs/sampling/2024-07-03/11-37-55",
                        "best_predictions_list_*.pkl",
                    ),
                    self.device,
                )
                labels_lists = load_pickle_lists(
                    os.path.join(
                        "outputs/sampling/2024-07-03/11-37-55", "labels_list_*.pkl"
                    ),
                    self.device,
                )

                labels = labels_lists  # (B, T, N, D)
                predictions = predictions_lists  # (B, n_samples, T, N, D)
                best_predictions = best_predictions_lists

            if self.args.use_distributed and not self.args.test_saved_data:
