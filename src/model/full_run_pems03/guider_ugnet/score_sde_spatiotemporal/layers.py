...
class AVWGCN(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        output = []
        for t in range(T):
            x_t = x[:, :, :, t]  # Select data at time t, shape [B, N, C]
            x_t = x_t.permute(0, 2, 1).contiguous()  # [B, N, C]
            ...
            x_g = x_g.permute(0, 2, 1, 3).contiguous()  # [B, N, cheb_k, C]
            x_gconv = torch.einsum("bnki,nkio->bno", x_g, weights) + bias
            x_skip = x_t
            x_gconv = x_gconv + x_skip
...
#         st_output = st_output.permute(0, 2, 1, 3)
...
class ResnetST(nn.Module):
    ...
    def __init__(
        ...
        self.GroupNorm_0 = nn.GroupNorm(num_groups=32, num_channels=in_ch, eps=1e-6)
        self.act = act
        self.Conv_0 = NewSpatioTemporalLayer(c_in=in_ch, c_out=out_ch)
        if temb_dim is not None:
            self.Dense_0 = nn.Linear(temb_dim, out_ch)
            self.Dense_0.weight.data = default_init()(self.Dense_0.weight.data.shape)
            nn.init.zeros_(self.Dense_0.bias)
        ...
        self.Conv_1 = NewSpatioTemporalLayer(c_in=out_ch, c_out=out_ch)

        if in_ch != out_ch:
            if conv_shortcut:
                self.Conv_2 = NewSpatioTemporalLayer(c_in=in_ch, c_out=out_ch)
            else:
                self.NIN_0 = NIN(in_ch, out_ch)
